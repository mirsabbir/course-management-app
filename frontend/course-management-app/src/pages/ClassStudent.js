...
function ClassStudent() {
  ...
  const fetchStudents = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:5181/api/classes/${classId}/students`,
        {
          headers: {
  ...
  const handleUnenrollStudent = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:5181/api/classes/${classId}/students/${studentToUnenroll}`, // Use studentToUnenroll in the URL
        {
  ...
  return (
    <Container style={{ marginTop: "20px" }}>
      ...
      <Typography variant="h4" gutterBottom textAlign="center">
        Manage Students for Class Name: {className}
      </Typography>
